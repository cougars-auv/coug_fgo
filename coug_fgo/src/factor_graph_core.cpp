...
FactorGraphCore::FactorGraphCore(const factor_graph_node::Params & params)
: params_(params) {}

static gtsam::SharedNoiseModel applyRobustKernel(
  ...
  const std::string & kernel, double k)
{
  if (kernel == "Huber") {
    ...
    return gtsam::noiseModel::Robust::Create(
      gtsam::noiseModel::mEstimator::Tukey::Create(k), noise);
  }
  return noise;
}

void FactorGraphCore::initialize(
...
FactorGraphCore::configureImuPreintegration(const utils::StateInitializer & state_init)
{
  ...
  if (params_.imu.use_parameter_covariance) {
    ...
    imu_params->gyroscopeCovariance = toGtsam(
      state_init.initial_imu_->angular_velocity_covariance);
  }
...
void FactorGraphCore::addPriorFactors(
  const utils::StateInitializer & state_init,
  ...
  const std::deque<nav_msgs::msg::Odometry::SharedPtr> & depth_msgs)
{
  ...
  depth_noise = applyRobustKernel(
    depth_noise, params_.depth.robust_kernel, params_.depth.robust_k);
...
void FactorGraphCore::addAhrsFactor(
  gtsam::NonlinearFactorGraph & graph,
  const std::deque<sensor_msgs::msg::Imu::SharedPtr> & ahrs_msgs)
{
  if (ahrs_msgs.empty()) {return;}
  ...
  ahrs_noise = applyRobustKernel(
    ahrs_noise, params_.ahrs.robust_kernel, params_.ahrs.robust_k);
  ...
  const std::deque<sensor_msgs::msg::MagneticField::SharedPtr> & mag_msgs)
{
  ...
  mag_noise = applyRobustKernel(
    mag_noise, params_.mag.robust_kernel, params_.mag.robust_k);
  ...
  const std::deque<geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr> & dvl_msgs)
{
  ...
  dvl_noise = applyRobustKernel(
    dvl_noise, params_.dvl.robust_kernel, params_.dvl.robust_k);
  ...
  const rclcpp::Time & target_time)
{
  ...
  dynamics_noise = applyRobustKernel(
    dynamics_noise, params_.dynamics.robust_kernel, params_.dynamics.robust_k);
  ...
  const rclcpp::Time & target_time)
{
  ...
  if (it_after == ahrs_msgs.begin()) {return toGtsam(ahrs_msgs.front()->orientation);}

  // If past the last message, extrapolate into the future
  if (it_after == ahrs_msgs.end()) {
    if (ahrs_msgs.size() < 2) {return toGtsam(ahrs_msgs.back()->orientation);}
    it_after--;
  }
  ...
  std::deque<geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr> & unused_dvl)
{
  ...
  graph.emplace_shared<DvlLoosePreintFactorArm>(
    ...
    tfs_.target_T_dvl,
    dvl_loose_preintegrator_->delta(),
  ...
  utils::QueueBundle & msgs)
{
  ...
  if (params_.ahrs.enable_ahrs) {addAhrsFactor(new_graph, msgs.ahrs);}
  ...
  auto addDropoutFactors = [&](gtsam::NonlinearFactorGraph & g) {
      ...
      bool use_const_vel = params_.const_vel.enable_const_vel ||
        params_.const_vel.enable_const_vel_dropout_only;
...
std::optional<OptimizeResult> FactorGraphCore::optimize()
{
  ...
  {
    std::scoped_lock update_lock(buffer_mutex);
    if (!has_buffer_) {return std::nullopt;}
